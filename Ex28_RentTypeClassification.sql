...
-- 문제
-- CAR_RENTAL_COMPANY_RENTAL_HISTORY 테이블에서 대여 시작일이 2022년 9월에 속하는 대여 기록에 대해서 대여 기간이 
-- 30일 이상이면 '장기 대여' 그렇지 않으면 '단기 대여' 로 표시하는 컬럼(컬럼명: RENT_TYPE)을 추가하여 
...
CREATE TABLE CAR_RENTAL_COMPANY_RENTAL_HISTORY1 (
...
INSERT INTO CAR_RENTAL_COMPANY_RENTAL_HISTORY1 (HISTORY_ID, CAR_ID, START_DATE, END_DATE)
...
SELECT HISTORY_ID, CAR_ID, START_DATE, END_DATE,
    CASE 
        ...
        ELSE '단기 대여'
    END AS RENT_TYPE
FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY1
